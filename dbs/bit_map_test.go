...
func TestBitMap_Has(t *testing.T) {
	...
	mydbs := dbs.NewBitMap[uint32]()

	for i := 0; i < 100000; i++ {
		tag := uint32(i)

		assert.False(t, mydbs.Has(tag))
		mydbs.Add(tag)
		assert.True(t, mydbs.Has(tag))
	}
...
func TestBitMap_DelBitMap(t *testing.T) {
	...
	ts1 := dbs.NewBitMap(2, 1, 4)
	ts2 := dbs.NewBitMap(2, 1, 3, 99)
	...
	ts1 = dbs.NewBitMap(2, 1, 3, 99)
	ts2 = dbs.NewBitMap(2, 1, 4)
...
func TestIntersection(t *testing.T) {
	...
	ts1 := dbs.NewBitMap(2, 1, 4, 7, 2, 4, 200)
	ts2 := dbs.NewBitMap(2, 1, 3, 4, 2, 18, 4)
	ts3 := dbs.NewBitMap(2, 1, 4, 3)
	tag := dbs.Intersection(ts1, ts2, ts3)
	...
	tag = dbs.Intersection(ts1)
	assert.Equals(t, tag, ts1)

	tag = dbs.Intersection[int]()
...
func TestBitMap_Load(t *testing.T) {
	...
	tag1 := dbs.NewBitMap(2, 18)
	tag2 := dbs.NewBitMap[int]()
